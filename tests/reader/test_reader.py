...
def test_ometiff_zarr(setup_mm2gamma_ome_tiffs):
...
def test_ometiff_array_zarr(setup_mm2gamma_ome_tiffs):
    _, _, rand_folder = setup_mm2gamma_ome_tiffs
    mmr = MicromanagerReader(rand_folder,
                             'ometiff',
                             extract_data=True)
...
def test_sequence_zarr(setup_mm2gamma_singlepage_tiffs):
...
def test_sequence_array_zarr(setup_mm2gamma_singlepage_tiffs):
    _, _, rand_folder = setup_mm2gamma_singlepage_tiffs
    mmr = MicromanagerReader(rand_folder,
                             'singlepagetiff',
                             extract_data=True)
