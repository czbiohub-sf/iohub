...
def test_init_array(setup_folder):
    ...
    writer.init_array(11, data_shape, chunk_size, dtype, chan_names, clims, position_name='Test', overwrite=False)
...
def test_write(setup_folder):
    ...
    assert(np.array_equal(writer.sub_writer.store['A']['1']['FOV1']['array'][0, 0, 0], data[0, 0, 0]))

    # Write full data
    writer.write(data, p=0)
    assert(np.array_equal(writer.sub_writer.store['A']['1']['FOV1']['array'][:, :, :, :, :], data))
