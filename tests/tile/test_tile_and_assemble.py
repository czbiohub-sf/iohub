...
def test_zyx_roundtrip_identity(synthetic_5d_large_z, tmp_path):
    """tile_and_assemble with ZYX tiling and identity fn preserves data."""
    result = tile_and_assemble(
        synthetic_5d_large_z,
        fn=lambda t: t,
        tile_size={"z": 8, "y": 32, "x": 64},
        output=str(tmp_path / "zyx_out.zarr"),
...
def test_zyx_roundtrip_gaussian(synthetic_5d_large_z, tmp_path):
    """ZYX tile_and_assemble with gaussian blending preserves identity."""
    result = tile_and_assemble(
        synthetic_5d_large_z,
        fn=lambda t: t,
        tile_size={"z": 8, "y": 32, "x": 64},
        output=str(tmp_path / "zyx_gauss.zarr"),
        overlap={"z": 2, "y": 8, "x": 16},
        weights="gaussian",
    )
    assert result.shape == synthetic_5d_large_z.shape
...
def test_zyx_with_memory_store(synthetic_5d_large_z):
    """ZYX tile_and_assemble works with zarr.Group on MemoryStore."""
    group = zarr.open_group(zarr.storage.MemoryStore(), mode="w")
    result = tile_and_assemble(
        synthetic_5d_large_z,
        fn=lambda t: t * 2,
    ...
    np.testing.assert_allclose(result.values, synthetic_5d_large_z.values * 2, atol=1e-5)
