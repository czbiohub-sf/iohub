...
def _adjust_chunks_for_divisibility(chunks: list[int], dims: list[int]) -> list[int]:
...
class TIFFConverter:
    ...
    def _gen_chunks(self, input_chunks):
        ...
        while chunks[-3] > 1 and np.prod(chunks, dtype=np.int64) * bytes_per_pixel > MAX_CHUNK_SIZE:
        ...
        for i, (orig, adj, dim) in enumerate(zip(original_chunks, chunks, data_dims)):
            if orig != adj:
